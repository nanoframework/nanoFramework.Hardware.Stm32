...
else
{
    ...
    git add "source\nanoFramework.Hardware.Stm32\Properties\AssemblyInfo.cs"
...
if ($env:APPVEYOR_REPO_TAG -eq "true")
{
    ...
    # new branch name
    $newBranch = "$env:APPVEYOR_REPO_BRANCH-nfbot/update-version/nanoFramework.Hardware.Stm32/$env:GitVersion_NuGetVersionV2"
    ...
    $assemblyFiles = (Get-ChildItem -Path ".\*" -Include "nf_hardware_stm32_native.cpp" -Recurse)
    ...
    {
        $commitMessage = "Update nanoFramework.Hardware.Stm32 version to $env:GitVersion_NuGetVersionV2"
